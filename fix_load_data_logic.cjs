...
const regex = /const loadData = useCallback\(async \(\) => \{[\s\S]*?\}, \[orderId, tenantId\]\);/;

const replaceStr = `const loadData = useCallback(async (isReload = false) => {
...
console.log("Fixed loadData function");
