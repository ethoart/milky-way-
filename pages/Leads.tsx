...
export const Leads: React.FC<LeadsProps> = ({ tenantId }) => {
  ...
  // Real-time history lookup on phone input change (Last 9 digits logic)
  useEffect(() => {
    const lookup = async () => {
  ...
  const handleManualSubmit = async () => {
    ...
    setMessage({ text: "Lead registered in cluster", type: 'success' });
              ...
              <div className="flex items-center justify-between pb-6 border-b border-slate-50">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Individual Entry</p>
                  </div>
                </div>
            ...
            <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm space-y-8">
              <div className="flex items-center justify-between pb-6 border-b border-slate-50">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">CSV Data Inbound</p>
                  </div>
                </div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <textarea 
                    ...
                    placeholder="Subject Name, Primary Phone, Full Address..."
                  />
                </div>
              </div>
              <button onClick={async () => {
                ...
                const orders = leads.map((l, i) => ({
                  ...
                  items: [{ productId: p.id, name: p.name, price: p.price, quantity: 1 }],
                  totalAmount: p.price, status: OrderStatus.OPEN_LEAD, createdAt: new Date().toISOString(),
                  isPrinted: false, openedBy: currentUser, logs: []
                }));
                await db.createOrders(orders);
                setMessage({ text: `Sync Complete: ${orders.length} leads injected`, type: 'success' });
                ...
                {isProcessing ? 'SYNCHRONIZING CLUSTER...' : 'EXECUTE BULK STREAM'}
              </button>
            </div>
        </div>

        {/* Right: History Intelligence Panel */}
                ...
                <div className="relative z-10">
                    ...
                    {!customerHistory ? (
                        <div className="flex-1 flex flex-col items-center justify-center text-center py-20 opacity-30">
                            ...
                            <p className="text-[10px] font-black uppercase tracking-[0.4em] leading-relaxed">Awaiting identity input<br/>to query global registry</p>
                        </div>
                    ) : (
                        <div className="space-y-10 animate-slide-in">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-white/5 p-6 rounded-3xl border border-white/10 text-center">
                                    <p className="text-[9px] font-black text-slate-500 uppercase mb-2 tracking-widest">Lifetime Leads</p>
                                ...
                                <div className={`p-6 rounded-3xl border text-center ${customerHistory.returns > 0 ? 'bg-rose-500/10 border-rose-500/30' : 'bg-emerald-500/10 border-emerald-500/30'}`}>
                                    <p className="text-[9px] font-black text-slate-500 uppercase mb-2 tracking-widest">Rejections</p>
                            ...
                            {customerHistory.returns > 0 && (
                                <div className="bg-rose-600/20 border border-rose-600/30 p-6 rounded-3xl flex items-start gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] font-bold text-rose-400 mt-1 uppercase leading-tight">
                                            High rejection risk. Subject has {customerHistory.returns} previous failures in the cluster.
                                        </p>
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="space-y-5 pt-8 border-t border-white/10">
                                <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-slate-500">
                                    <span>Protocol Identity</span>
                                    <span className="text-blue-400">*{manualForm.phone.replace(/\D/g, '').slice(-9)}</span>
                                </div>
                                <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-slate-500">
                                    <span>Current Status</span>
                                    <span className={`px-3 py-1 rounded-lg ${customerHistory.returns > 0 ? 'bg-rose-600/20 text-rose-500' : 'bg-emerald-600/20 text-emerald-500'}`}>{customerHistory.status}</span>
                                </div>
                                <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-slate-500">
                                    <span>Trust Score</span>
                                    <span className="text-white">{customerHistory.returns === 0 ? (customerHistory.count > 3 ? 'LEGACY' : 'NEUTRAL') : 'LOW'}</span>
                                </div>
                            </div>
                            ...
                            <div className="p-6 bg-blue-600/10 border border-blue-500/20 rounded-[2rem]">
                                <p className="text-[9px] font-black text-blue-400 uppercase tracking-widest flex items-center gap-2 mb-2">
                                    <Activity size={12}/> Analysis Insight
                                </p>
                                <p className="text-[11px] font-bold text-slate-300 leading-relaxed uppercase">
                                    {customerHistory.count === 0 
                                      ? "New subject detected. No historical data found in cluster nodes."
                                      : `Detected ${customerHistory.count} previous sessions. Verified node identifier matches.`
                                    }
                                </p>
                            </div>
                        </div>
