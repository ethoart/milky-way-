...
import { Search, ChevronRight, Layers, AlertCircle, Zap, Trash2, CheckSquare, Square, Truck } from 'lucide-react';
...
export const OrderList: React.FC<OrderListProps> = ({ 
  ...
  const loadData = async () => {
    ...
    const uniquePhones = [...new Set(fetchedOrders.slice(0, 20).map(o => o.customerPhone))];
    for (const phone of uniquePhones) { 
      const h = await db.getCustomerHistory(phone, tenantId);
      setCustomerHistories(prev => ({ ...prev, [phone]: h }));
    }
  };
  ...
  const handleDelete = async (e: React.MouseEvent, orderId: string) => {
    e.stopPropagation();
    if (!confirm("Milky Way: Permanently delete this record?")) return;
  ...
  return (
    <div className="flex flex-col h-full bg-white animate-slide-in relative">
      ...
      {selectedIds.length > 0 && onBulkAction && (
        <div className="absolute top-0 left-0 right-0 z-20 bg-slate-900 text-white p-4 flex items-center justify-between animate-slide-in shadow-2xl">
      ...
      <div className="p-5 border-b border-slate-100 flex flex-col md:flex-row items-center justify-between gap-4 bg-slate-50/20">
        ...
        <div className="flex items-center gap-3">
          <button onClick={toggleAll} className="flex items-center gap-2 px-4 py-2 bg-slate-100 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-600 hover:bg-slate-200 transition-all">
            {selectedIds.length === filteredOrders.length ? <CheckSquare size={14}/> : <Square size={14}/>} {selectedIds.length === filteredOrders.length ? 'Deselect All' : 'Select Page'}
          </button>
